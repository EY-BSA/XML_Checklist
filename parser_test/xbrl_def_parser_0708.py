"""
xbrl_def_parser_0708.py
XBRL ZIP → def.xml 기준 fact 추출기

def.xml 하이퍼큐브(표→축→멤버)로 유효성을 검증한 fact를
사람이 표를 읽는 순서(주석 → 표 → 멤버 → 라인아이템 → 당기/전기)로 나열해
엑셀 'result' 시트 하나로 저장한다.

실행:
    python3 parser_test/xbrl_def_parser_0708.py [zip경로]
"""
...
# ── 실행: def.xml 기준 fact 추출 → 엑셀 'result' 시트 ─────────────────────────
...
if __name__ == '__main__':
    ...
    with open(zip_path, 'rb') as fp:
        data = parse_xbrl_zip(fp.read())

    rows = data.presentation_rows
    print('회사:', data.company_name, '|', data.fy, data.report_period)
    if data.errors:
        print('errors:', data.errors)

    # 자가 검증
    assert rows, 'presentation 파싱 결과 없음'
    assert data.facts, '인스턴스에서 fact를 찾지 못함'

    # ── 정렬·추출 ──
    # 정렬: 재무제표(주석번호 없음 → 0) 먼저, 그다음 주석 번호 순 ('17-1.' 형식 지원).
    # 같은 주석은 role 코드 순(연결 → 별도).
    def _note_no(r: dict) -> tuple[int, int]:
    ...
    # 멤버별 fact 전개 시트 (def.xml 기준):
    # LINEITEM별로 def.xml상 유효한 fact를 전기 포함 전부 나열.
    # 사람이 표를 읽는 순서로 정렬: 표 등장 순서 → 멤버(presentation 순서) →
    # 라인아이템(presentation 순서) → 기간(당기→전기)
    by_name = defaultdict(list)
    for f in data.facts:
        by_name[f['Name']].append(f)
    max_year = _current_year(data.contexts)

    # presentation 순서 인덱스: 표 / 표 내 멤버 / 라인아이템, 그리고 한/영 라벨 맵
    table_order:  dict = {}
    member_order: dict = {}
    label_ko:     dict = {}
    label_en:     dict = {}
    name_to_id:   dict = {}   # 개념명 → '프리픽스_개념명' (XSD 요소 id)
    for i, r in enumerate(rows_sorted):
        label_ko.setdefault(r['Name'], r['Label(KO)'])
        label_en.setdefault(r['Name'], r['Label(EN)'])
        name_to_id.setdefault(
            r['Name'], f"{r['Prefix']}_{r['Name']}" if r['Prefix'] else r['Name'])
        tkey = (r['Sheet'], r.get('xbrl_table_name', ''))
    ...
    for li_idx, r in enumerate(rows_sorted):
        if r['구분'] != 'LINEITEM':
            continue
        tkey = (r['Sheet'], r.get('xbrl_table_name', ''))
        for f, ctx in _fact_candidates(r, by_name, data.contexts, max_year,
                                       data.def_map, all_periods=True):
            dims = {k: v for k, v in ctx['dims'].items() if k != _CONSOL_AXIS}
            # 멤버 정렬키: presentation에서 그 멤버가 나온 위치 (총계=멤버 없음 → 맨 앞)
            mem_key = tuple(sorted(member_order.get((*tkey, m), 10**9)
                                   for m in dims.values()))
            period_rank = int(max_year) - int(ctx['end'][:4] or 0)
    ...
    keyed_rows.sort(key=lambda kr: kr[0])
    fact_rows = [d for _, d in keyed_rows]

    import pandas as pd
    out_path = Path(__file__).parent / f'def_result_{Path(zip_path).stem[:20]}.xlsx'
    pd.DataFrame(fact_rows).to_excel(out_path, sheet_name='result', index=False)
    print(f'엑셀 저장: {out_path}  (result {len(fact_rows)}행)')
    print('OK')
