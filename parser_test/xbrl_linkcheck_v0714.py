"""
xbrl_linkcheck_v0714.py
XBRL ZIP → def.xml 기준 fact 추출기 (CY/PY/BY 컬럼 방식, 정확한 key 매핑)

v0710 대비 변경:
  - _fact_candidates()의 "dimension 수 적은 것 우선" 추정 로직 제거
  - fact를 (Name, dims_frozenset, year, consol) key로 인덱싱 → 1:1 정확 매핑
  - def.xml 허용 조합에 정확히 일치하는 fact만 선택

실행:
    python3 parser_test/xbrl_linkcheck_v0714.py [zip경로] [출력경로.xlsx]
"""
...
_META_COLS = [
    ...
    '축', '축(EN)', '축ID', '멤버(KO)', '멤버(EN)', '멤버ID', '총계여부',
...
def parse_xbrl_zip_wide(file_bytes: bytes) -> list[dict]:
    ...
    fact 매핑 방식:
      (Name, dims_frozenset, year, consol) 키로 1:1 정확 매핑.
      def.xml 허용 조합에 정확히 일치하는 fact만 포함 (추정 없음).
    ...
    # ── fact 인덱스: (Name, dims_fs, year, consol) → (fact, ctx) ──────────────
    # 동일 key 중복 시 첫 번째 유지
    fact_idx: dict[tuple, tuple] = {}
    by_name:  dict[str, list]   = defaultdict(list)
    for f in data.facts:
        by_name[f['Name']].append(f)
        ctx = data.contexts.get(f['contextRef'])
        if not ctx:
            continue
        consol   = ctx['dims'].get(_CONSOL_AXIS, '')
        other_fs = frozenset((k, v) for k, v in ctx['dims'].items() if k != _CONSOL_AXIS)
        year     = ctx['end'][:4]
        fact_idx.setdefault((f['Name'], other_fs, year, consol), (f, ctx))
    ...
    for li_idx, r in enumerate(rows_sorted):
        ...
        tbl_axes  = data.def_map.get(role_code, {}).get(tname, {})

        # 연결/별도 context 값 결정
        if r.get('is_consolidated') is True:
            consol_val = 'ConsolidatedMember'
        elif r.get('is_consolidated') is False:
            consol_val = 'SeparateMember'
        else:
            consol_val = ''

        # ── 정확한 매핑: by_name에서 후보 꺼낸 뒤 def.xml 정확 검증 ──────────
        # dims_fs → {year: (fact, ctx)}
        dim_year: dict[frozenset, dict[str, tuple]] = defaultdict(dict)
        ...
        for f in by_name.get(name, []):
            ...
            # 연결/별도 필터
            consol = ctx['dims'].get(_CONSOL_AXIS, '')
            if consol_val and consol != consol_val:
                continue

            other_dims = {k: v for k, v in ctx['dims'].items() if k != _CONSOL_AXIS}

            # def.xml 정확 검증: context의 모든 dim이 허용 조합인지 확인
            # (추정 없음 — 허용 여부만 판단, 우선순위 없음)
            if tbl_axes:
                ok = all(
                    tbl_axes.get(ax) is not None and mem in tbl_axes[ax]
                    for ax, mem in other_dims.items()
                )
                if not ok:
                    continue
            elif other_dims:
                # def.xml 없는데 extra dims 있는 경우 → 스킵
                continue

            dims_fs = frozenset(other_dims.items())
            year    = ctx['end'][:4]
            # 같은 (dims_fs, year) 조합 중복 시 첫 번째 유지
            dim_year[dims_fs].setdefault(year, (f, ctx))

        # periodStartLabel 행(기초)은 전년도 기말 시점 fact가 그 값이므로
        # 연도 매핑을 한 해 당긴다. 단, 이름에 BeginningOfPeriod가 든 전용
        # 기초 개념(현금흐름표 등)은 당기 컨텍스트에 태깅되므로 시프트 제외.
        pref = (r.get('PreferredLabel') or '').lower()
        if 'periodstart' in pref and 'beginning' not in name.lower():
            row_years = (py_year, by_year,
                         all_years[3] if len(all_years) > 3 else '')
        else:
            row_years = (cy_year, py_year, by_year)

        # ── dims_fs별로 CY/PY/BY 묶어서 출력 행 생성 ─────────────────────────
        for dims_fs, year_map in dim_year.items():
            cy = year_map.get(row_years[0])
            py = year_map.get(row_years[1])
            by = year_map.get(row_years[2])
            if not (cy or py or by):
                continue

            dims     = dict(dims_fs)
            # 축이 있는 표인데 멤버 없는 fact로 채워진 행 = 전사 총계가
            # 표에 끼어든 것일 수 있음 (ex. 수준3 변동표의 부채총계) → 플래그
            has_other_axes = any(ax != _CONSOL_AXIS for ax in tbl_axes)
            total_flag = '총계(멤버없음)' if (not dims and has_other_axes) else ''
            ...
            meta = (
                ...
                r.get('Label(EN)', ''),
                name,
                f"{r['Prefix']}_{name}" if r.get('Prefix') else name,
                r.get('Arcrole', ''),
                r.get('연결/별도', ''),
            )
            grouped.setdefault((sort_key, meta), {})[cy_year] = cy
            if py: grouped[(sort_key, meta)][py_year] = py
            if by: grouped[(sort_key, meta)][by_year] = by
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('Usage: python xbrl_linkcheck_v0714.py <zip_path> [output.xlsx]')
        sys.exit(1)
