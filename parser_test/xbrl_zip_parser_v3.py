...
def _remove_def_invalid_rows(rows: list, lineitem_map=None) -> list:
    """def.xml에 등록되지 않은 DOMAIN/MEMBER/LINEITEM 행 제거.

    [DOMAIN/MEMBER]
    축_도메인=None 이면서 Element가 Domain/Member이면
    def.xml 기준 현재 table+axis에 속하지 않는 보일러플레이트로 판단.

    [LINEITEM]
    lineitem_map이 있을 때,
    role_code + xbrl_table_name 기준 유효 LINEITEM 집합에 없는 항목 제거.
    """
    def _is_invalid(row):
        # DOMAIN / Member 보일러플레이트
